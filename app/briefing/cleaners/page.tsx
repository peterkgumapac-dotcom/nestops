            ...
            {prefs?.toggles.propertiestoday && (
              sortedShifts.length === 0 ? (
                ...
                sortedShifts.map((shift, idx) => {
                  ...
                  const propData = PROPERTIES.find(p => p.id === shift.propertyId)
                  const accessCode = propData?.accessCodes?.[0]?.code ?? 'Check SuiteOp'
                  const codeVisible = accessCodeVisible[shift.id] ?? false
                    ...
                    <div key={shift.id} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 16, padding: '16px', marginBottom: 12, overflow: 'hidden' }}>
                      ...
                      {prefs?.toggles.accesstype && (
                        codeVisible ? (
                          ...
                          <button
                            onClick={() => setAccessCodeVisible(prev => ({ ...prev, [shift.id]: true }))}
            ...
            {myJobs.length > 0 && (
              <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, padding: '10px 14px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)' }}>
                  {myJobs.length} task{myJobs.length !== 1 ? 's' : ''} in your queue today
                </span>
